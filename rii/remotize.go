...
func remotize(it *reflect.InterfaceType, pack string) {
	...
	gf.AddComment("RII-Autogenerated " + ifacename + " RPC wrapper [DO NOT EDIT!]")
	gf.AddComment("Server wrapper for " + ifacename)
	gf.DeclType(Ifacename+"Server", "struct {\n\ti *"+Ifacename+"\n}")
	gf.AddComment("Client wrapper for " + ifacename)
	gf.DeclType(Ifacename+"Client", "struct {\n\tc *rii.Client\n}")
	for i := 0; i < nm; i++ {
		m := it.Method(i)
		gf.DeclMethod("s *"+Ifacename+"Server", m.Name,
			"a *Args, r *Results", "os.Error")
	}
	gf.Debug()
}
/*
