...
// Remotize will create the stub and skel files needed to use some given 
// interface remotely
...
func buildStubSkel(it *reflect.InterfaceType) {
	iface:=it.Name()
	fst:=buildHeader("Stub",iface)
	if(fst==nil) {
		return
	}
	fsk:=buildHeader("Skel",iface)
	if(fsk==nil) {
		return
	}
	nm:=it.NumMethod()
	for i:=0;i<nm;i++ {
		m:=it.Method(i)
		buildStubMethod(iface,i,fst,m)
	}
	buildSkelExecutor(it,fsk)
	fsk.Close()
	fst.Close()
}

func buildHeader(side string, iface string) *os.File {
	filename:=iface+side+".go"
	f,err:=os.Open(filename,os.O_CREAT|os.O_WRONLY,0644)
	if(err!=nil) {
		fmt.Println("Can't open ",filename,"for writing:",err)
		return nil
	}
	fmt.Fprintf(f,"package rii\n\n")
	fmt.Fprintf(f,"// Autogenerated "+iface+" "+side+" [DO NOT EDIT]\n\n")
	fmt.Fprintf(f,"type "+iface+side+" "+strings.ToLower(side)+";\n\n")	
	return f
}

func buildStubMethod(iface string,n int,f *os.File,m reflect.Method) {
...
func buildSkelExecutor(it *reflect.InterfaceType,f *os.File) {
	iface:=it.Name()
	fmt.Fprintf(f,"func (sk *"+iface+"Skel) execute(funcNum int, ")
	fmt.Fprintf(f,"args []interface{}) []interface{} {\n")
	fmt.Fprintf(f,"\tswitch funcNum {\n")
	nm:=it.NumMethod()
	for i:=0;i<nm;i++ {
