...
// A remote reference to a type or interface
type Ref struct {
	name string
	addr net.Addr
}
...
// NewService returns a new service wrapper to serve calls to 'ifaceimpl' remotely
...
// NewRef returns a new Remote Reference to a Remote interface or type
func NewRef(iface interface{}, addr net.Addr) Ref {
	return Ref{nameFor(iface),addr}
}

// NewRemoteByRef gets a proxy to a remote interface by itr Remote Reference
func NewRemoteByRef(ref Ref) interface{} {
	p := RegistryFind(searchName("Remote", nameFor(ref.name)))
	if p == nil {
		return nil
	}
	client, e := rpc.DialHTTP(ref.addr.Network(), ref.addr.String())
	if e!=nil {
		panic(e)
	}
	return p.(BuildRemote)(client)
}
