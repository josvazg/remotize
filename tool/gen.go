...
func NewSpec(packname string, isInterface bool, i interface{}) *Spec {
	...
	return &Spec{packname, isInterface, t, make(map[string]string), bytes.NewBufferString("")}
}

// Turns a type into a Spec to be remotized
func Type2Spec(isInterface bool, t reflect.Type) *Spec {
	return &Spec{t.PkgPath(), isInterface, t, make(map[string]string), bytes.NewBufferString("")}
}
...
func genWrapper(name string, spec *Spec) os.Error {
	...
	fmt.Fprintf(spec.src, "type %s interface", ifacename)
	spec.methods()
	hdr:=spec.header()
	fmt.Fprintf(f, hdr)
	if e := doremotize(spec.t, f, hdr+spec.src.String()); e != nil {
		return e
	}
	f.Close()
	return nil
...
func (s *Spec) funcsource(t reflect.Type, m *reflect.Method) {
	start := 0
	if t.Kind() == reflect.Interface {
		fmt.Fprintf(s.src, m.Name+"(")
	} else if m != nil && m.Name != "" {
		start++
		fmt.Fprintf(s.src, m.Name+"(")
	} else {
		fmt.Fprintf(s.src, "func (")
	}
	if m != nil {
		t = m.Type
	}
...
func (s *Spec) typesource(t reflect.Type) {
	switch t.Kind() {
	case reflect.Array:
		fmt.Fprintf(s.src, "["+strconv.Itoa(t.Len())+"]")
		s.typesource(t.Elem())
	case reflect.Chan:
		fmt.Fprintf(s.src, "chan ")
		s.typesource(t.Elem())
	case reflect.Func:
		s.funcsource(t, nil)
	case reflect.Map:
		fmt.Fprintf(s.src, "map[")
		s.typesource(t.Key())
		fmt.Fprintf(s.src, "]")
		s.typesource(t.Elem())
	case reflect.Ptr:
	...
	case reflect.Slice:
		...
		s.typesource(t.Elem())
	case reflect.String:
		fmt.Fprintf(s.src, "string")
	default:
		s.pack(t)
		fmt.Fprintf(s.src, t.String())
		return
	}
}
...
func generateRemotizerCode(d *Detected) string {
	...
	for name, dcl := range d.RDecls {
		fmt.Fprintf(src, "\n\ttool.Spec{\"%v\",", d.packname)
		fmt.Fprintf(src, "\n\t\t%v,", dcl.isInterface)
		fmt.Fprintf(src, "\n\t\tnew(%v),", ifacename(name))
		fmt.Fprintf(src, "\n\t},")
	}
